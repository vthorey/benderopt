...
def test_categorical_generator():
    ...
    epsilon = 1e-2
    samples = list(
        sample_generators["categorical"](values=values, probabilities=probabilities, size=size)
    )
...
def test_categorical_pdf():
    ...
    densities = probability_density_function["categorical"](
        values=values, probabilities=probabilities, samples=samples
    )
