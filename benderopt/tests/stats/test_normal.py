...
def test_normal_generator():
    ...
    samples = sample_generators["normal"](
        size=size, mu=mu, sigma=sigma, low=low, high=high, step=step
    )
    a = (low - mu) / sigma
    b = (high - mu) / sigma
    # Median
...
def test_normal_pdf():
    ...
    epsilon = 1e-1
    samples = sample_generators["normal"](
        size=size, low=low, high=high, step=step, mu=mu, sigma=sigma
    )
...
def test_normal_pdf_step():
    ...
    densities = probability_density_function["normal"](
        samples=bin_edges[0:-1] + step / 10, low=low, high=high, step=step, mu=mu, sigma=sigma
    )
    assert np.sum(densities[(bin_edges[1:] + bin_edges[:-1]) * 0.5 < low]) == 0
    assert np.sum(densities[(bin_edges[1:] + bin_edges[:-1]) * 0.5 < low]) == 0
    assert ((hist - densities) / densities).mean() <= epsilon
